...
def table_string (list_rows):
    # generates HTML table from 2 rows
    mask="<td>{}</td>"  
    table_s="<table>"
...
def html_user_info_with_context(dataset_path, dic_map_users, dic_map_items, dic_map_channels,  users_real, items_real, \
...
def info_model_report (model, dataset_path, res_info, \
    ...
    # generates customized report that shows different metrics / results of the model, including images of purchased / recommended products
...
def info_model_report_with_context (model, dataset_path, res_info, \
                l_info, full_dataset, dict_recomen, title="None", topk=10):
    # generates customized report that shows different metrics / results of the model, including images of purchased / recommended products
    ...
    for n in range (0,NUMBERFIRSTSAMPLES):
        ...
        l_recommened_list_user=l_recommened_list[n]
        html_user= html_user_info_with_context (dataset_path,dic_map_users, dic_map_items, dic_map_channels, users_real, items_real,\
                 df_iter, user, gt, gt_channel, l_recommened_list_user )
        html_out+=html_user
