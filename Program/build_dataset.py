...
def extract_dictionary(transaction):
    # extracts customers and articles dictionaries
    cust_dict = {};
    art_dict = {}
    count_cust = 1;
    count_art = 1;
    for index, row in transaction.iterrows():
        ...
        if (customer not in cust_dict):
            cust_dict[customer] = count_cust;
            count_cust += 1
        if (article not in art_dict):
            art_dict[article] = count_art;
            count_art += 1
...
def generate_datasets(transaction, cust_dict, art_dict):
    ...
    for index, row in transaction.iterrows():
        # extracts customer and article from transaction records
        customer = row["customer_id"];
...
class CustomerArticleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, num_negatives_train=4, num_negatives_test=100, sep='\t'):
        ...
        # reindexing articles...
        self.targets = train_data[:, 2] # extracts articles from train dataset
        self.items = self.preprocess_items(train_data,
                                           number_customers) # reindexes data
        ...
        # builds complete test set by passing as input the test item interactions
        self.test_set_AllItems = self.build_test_set_AllItems(self.preprocess_items(test_data, number_customers))
    ...
    def preprocess_items(self, data, num_customers):
        ...
        reindexed_items = data[:, :2].astype(np.int) - 1
    ...
    def build_test_set(self, gt_test_interactions, num_neg_samples_test):
        ...
        max_users, max_items = self.field_dims[:2]
        test_set = []
        for pair in tqdm(gt_test_interactions, desc="BUILDING PARTIAL TEST SET..."):
            negatives = []
    ...
    def build_test_set_AllItems(self, gt_test_interactions):
        # builds complete test set
        max_users, max_items = self.field_dims[:2]
        test_set_allitems = []
        for pair in tqdm(gt_test_interactions, desc="BUILDING COMPLETE TEST SET..."):
            negatives = []
            # loops all the users , items
            for t in range(max_users, max_items):
                if (pair[0], t) in self.train_mat or t == pair[1]:
                    None
                else:
                    negatives.append(t)
            single_user_test_set = np.vstack([pair, ] * (len(negatives) + 1))
            single_user_test_set[:, 1][1:] = negatives
            test_set_allitems.append(single_user_test_set.copy())
        return test_set_allitems
...
if __name__ == '__main__':
    ...
    # extracts customers and article dictionaries
