...
if __name__ == '__main__':
    ...
    hr, ndcg, cov, gini, dict_FM, nov = testfull(model_FM, full_dataset, device, topk=topk)
    if epoch_i == num_epochs - 1:
        new_row = pd.Series(data={'Dataset': 'H&M', 'Model': 'FM', 'HR': hr, 'NDCG': ndcg, 'COVERAGE': cov, 'GINI': gini,
                                  'NOVELTY': nov, 'TESTSET': 'COMPLETE'})
        metrics_model = metrics_model.append(new_row, ignore_index=True)
    ...
    hr, ndcg, cov, gini, dict_GCN, nov = testfull(model_GCN, full_dataset, device, topk=topk)
    if epoch_i == num_epochs - 1:
        new_row = pd.Series(data={'Dataset': 'H&M', 'Model': 'GCM', 'HR': hr, 'NDCG': ndcg, 'COVERAGE': cov, 'GINI': gini,
                                  'NOVELTY': nov, 'TESTSET': 'COMPLETE'})
        metrics_model = metrics_model.append(new_row, ignore_index=True)
    ...
    hr, ndcg, cov, gini, dict_GCN_att, nov = testfull(model_GCN_att, full_dataset, device, topk=topk)
    if epoch_i == num_epochs - 1:
        new_row = pd.Series(data={'Dataset': 'H&M', 'Model': 'GCN_ATT', 'HR': hr, 'NDCG': ndcg, 'COVERAGE': cov, 'GINI': gini,
                                  'NOVELTY': nov, 'TESTSET': 'COMPLETE'})
        metrics_model = metrics_model.append(new_row, ignore_index=True)
    ...
    hr, ndcg, cov, gini, dict_rand, nov = testfull(model_rand, full_dataset, device, topk=topk)
    if epoch_i == num_epochs - 1:
        new_row = pd.Series(data={'Dataset': 'H&M', 'Model': 'Random', 'HR': hr, 'NDCG': ndcg, 'COVERAGE': cov, 'GINI': gini,
                                  'NOVELTY': nov, 'TESTSET': 'COMPLETE'})
        metrics_model = metrics_model.append(new_row, ignore_index=True)
    ...
    hr, ndcg, cov, gini, dict_pop, nov = testfull(model_pop, full_dataset, device, topk=topk)
    if epoch_i == num_epochs - 1:
        new_row = pd.Series(data={'Dataset': 'H&M', 'Model': 'Popularity', 'HR': hr, 'NDCG': ndcg, 'COVERAGE': cov, 'GINI': gini,
                                  'NOVELTY': nov, 'TESTSET': 'COMPLETE'})
        metrics_model = metrics_model.append(new_row, ignore_index=True)
    ...
    print(metrics_model)
    metrics_model.to_excel("../data/metrics.xlsx")
