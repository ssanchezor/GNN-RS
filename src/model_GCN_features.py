import torch
import numpy as np
from model_FM_features import FeaturesLinear, FieldAwareFactorizationMachine
...
class FactorizationMachineModel_withGCN(torch.nn.Module):
    def __init__(self, field_dims, embed_dim, X, A, attention=False):
        ...
        self.embeddings = torch.nn.ModuleList([
            GraphModel(field_dims, embed_dim, X, A, attention=attention) for _ in range(self.num_fields)
        ])
        self.fm = FieldAwareFactorizationMachine(reduce_sum=True)

    def forward(self, interaction_pairs):
        out = self.linear(interaction_pairs) + self.fm(self.embeddings(interaction_pairs))
